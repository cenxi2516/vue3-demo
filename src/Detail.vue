...
<template>
	<a-avatar class="my-4" shape="square" :size="128" :src="dataSource?.avatar" alt="头像"> </a-avatar>
	<a-descriptions v-if="void 0 !== dataSource" :column="{ xs: 1, sm: 2 }">
		<a-descriptions-item label="账号">
			{{ dataSource.userName }}
			<a-tag class="ml-4" :color="STATE_MAP[dataSource.state][0]"> {{ STATE_MAP[dataSource.state][1] }}</a-tag>
		</a-descriptions-item>
		...
		<a-descriptions-item label="角色">
			<a-tag v-for="(role, i) in dataSource.roles" :key="i">{{ role }}</a-tag>
		</a-descriptions-item>
	</a-descriptions>
</template>
